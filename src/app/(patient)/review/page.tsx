...
function InterventionCard({ intervention }: { intervention: Intervention }) {
  ...
  if (intervention.kind === 'approval') {
    return (
      <div className="bg-dialogue-surface border border-dialogue-border rounded-cardLarge p-8 flex flex-col items-center text-center">
        <CheckCircle2 className="w-14 h-14 text-green-400 mb-4" />
        <LottiePharmacist loop={false} className="w-36 h-36 mb-4" />
        <h2 className="font-display-bold text-sectionTitle text-dialogue-text mb-3">
        ...
        <p className="text-xs text-dialogue-textMuted font-body mt-4 italic">
          Tucker et al., 2017
        </p>
      </div>
    )
  ...
  if (intervention.kind === 'phone_call') {
    return (
      <div className="bg-dialogue-surface border border-dialogue-border rounded-cardLarge p-8 flex flex-col items-center text-center">
        <Phone className="w-12 h-12 text-dialogue-accent mb-4" />
        <h2 className="font-display-bold text-sectionTitle text-dialogue-text mb-2">
          Your pharmacist will call you
        </h2>
        {payload.phone_time && (
          <p className="font-body text-body text-dialogue-textMuted">
            Scheduled for{' '}
            <span className="text-dialogue-text font-body-bold">{payload.phone_time}</span>
          </p>
        )}
      </div>
    )
  ...
  if (intervention.kind === 'clinical_note') {
    return (
      <div className="bg-dialogue-surface border border-dialogue-border rounded-cardLarge p-8">
        <div className="flex items-center gap-3 mb-4">
          <FileText className="w-8 h-8 text-dialogue-accent" />
          <h2 className="font-display-bold text-sectionTitle text-dialogue-text">
            Note from your pharmacist
          </h2>
        </div>
        {payload.note_text && (
          <p className="font-body text-body text-dialogue-textMuted leading-relaxed whitespace-pre-wrap">
            {payload.note_text}
          </p>
        )}
      </div>
  ...
  if (intervention.kind === 'in_person') {
    return (
      <div className="bg-dialogue-surface border border-dialogue-border rounded-cardLarge p-8 flex flex-col items-center text-center">
        <CalendarDays className="w-12 h-12 text-dialogue-accent mb-4" />
        <h2 className="font-display-bold text-sectionTitle text-dialogue-text mb-2">
          Please come in
        </h2>
        {payload.appointment_date && (
          <p className="font-body text-body text-dialogue-textMuted">
            ...
            <span className="text-dialogue-text font-body-bold">{payload.appointment_date}</span>
          </p>
        )}
      </div>
    )
  ...
  return (
    <div className="flex flex-col items-center justify-center py-16">
      <div className="relative mb-6">
        <div className="w-16 h-16 rounded-full border-4 border-dialogue-border" />
        <div className="absolute inset-0 w-16 h-16 rounded-full border-4 border-t-dialogue-accent animate-spin" />
      </div>
      <h2 className="font-display-bold text-sectionTitle text-dialogue-text mb-2">
        Waiting for your pharmacist…
      </h2>
      <p className="font-body text-body text-dialogue-textMuted text-center max-w-xs">
...
function ReviewContent() {
  ...
  return (
    <main className="min-h-screen bg-dialogue-bg px-screenX pt-screenTop pb-20">
      <h1 className="font-display-bold text-screenTitle text-dialogue-text mb-8">Review</h1>
    ...
    <Suspense
      fallback={
        <main className="min-h-screen bg-dialogue-bg px-screenX pt-screenTop pb-20 flex items-center justify-center">
          <p className="font-body text-body text-dialogue-textMuted animate-pulse">Loading…</p>
        </main>
      }
    >
      <ReviewContent />
